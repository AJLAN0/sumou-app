...
3. Authorize: `auth.getUser()` → the caller's OWN `profiles` self-read (returns a
   row only when active + not soft-deleted) → also yields the username. Fail-closed
   (read error → 500).
...
deno check supabase/functions/change-own-password/index.test.ts
deno test --allow-env supabase/functions/change-own-password/index.test.ts
```
Results: `fmt --check` OK; `deno check` OK (both files); `deno test`
**21 passed / 0 failed**. Covered: method/content-type/bearer/body framing, strict
allowlist, the full new-password policy + same-password rejection (all pre-network),
own-active-staff authorization (unauthenticated/inactive/missing/query-error),
current-password re-auth (correct/wrong/no-session/thrown), Auth update on the exact
caller uid, RPC only after a successful Auth update, RPC skipped on Auth failure,
partial-failure 500, and no-secret-leak assertions on every response body.
